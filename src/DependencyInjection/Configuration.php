...
final class Configuration implements ConfigurationInterface
{
            ...
            ->children()
                ...
                ->arrayNode('notifier')
                    ...
                    ->children()
                        ->scalarNode('service')
                            ...
                            ->cannotBeEmpty()
                            ->info('The notifier service to use')
                        ->end()
                        ->arrayNode('default_channel')
                            ->beforeNormalization()->castToArray()->end()
                            ->scalarPrototype()->end()
                            ->info('The default notifier channel to use')
                        ->end()
                        ->scalarNode('default_email')
                            ->info('The default email to use for email notification')
                            ->defaultNull()
                        ->end()
                        ->scalarNode('default_phone')
                            ->info('The default phone number to use for SMS notification')
                            ->defaultNull()
                        ->end()
                        ->scalarNode('subject_prefix')
                            ->info('The prefix to use for notifier subjects (use to distinguish between different application schedules)')
                            ->example('"[Acme Inc Website]"')
                            ->defaultNull()
                        ->end()
                    ->end()
                ...
                ->arrayNode('schedule_extensions')
                    ...
                    ->children()
                        ...
                        ->append(self::createNotifyExtension('notify_on_failure', 'Send notification if schedule fails'))
    ...
    private static function taskConfiguration(): ArrayNodeDefinition
    {
        ...
        $node
            ...
            ->arrayPrototype()
                ->validate()
                    ->ifTrue(static fn($v) => [null] === $v['task'] && !$v['description'])
                ...
                ->children()
                    ...
                    ->append(self::createNotifyExtension('notify_after', 'Send notification after task runs'))
                    ->append(self::createNotifyExtension('notify_on_failure', 'Send notification if task fails'))
                ->end()
            ->end()
        ;
    ...
    private static function createNotifyExtension(string $name, string $description): ArrayNodeDefinition
    {
        $treeBuilder = new TreeBuilder($name);
        $node = $treeBuilder->getRootNode();
        ...
        $node
            ...
            ->children()
                ->arrayNode('channel')
                    ->beforeNormalization()->castToArray()->end()
                    ->scalarPrototype()->end()
                    ->info('Channel to send notification to (leave blank to use "zenstruck_schedule.notifier.default_channel")')
                ->end()
                ->scalarNode('email')
                    ->info('Email address for email notifications (leave blank to use extension default)')
                    ->defaultNull()
                ->end()
                ->scalarNode('phone')
                    ->info('Phone number for SMS notifications (leave blank to use extension default)')
                    ->defaultNull()
                ->end()
                ->scalarNode('subject')
                    ->info('Notification subject (leave blank to use extension default)')
                    ->defaultNull()
                ->end()
            ->end()
        ;
